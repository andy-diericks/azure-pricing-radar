...
## 2026-07-20T14:22Z — run 2026-07-20-5
- Task: #75 E2.1a — Build a compact sku-index.json from data/
- Did: Created `app/scripts/build-sku-index.js` (Node ESM) with two exported pure functions: `toSlug(skuName)` (URL-safe slug, e.g. "Standard_D2s_v5" → "standard-d2s-v5") and `buildSkuIndex(latestByScope, diffs)` (replays all diff files chronologically to build per-key price history, then groups by skuName slug). Added `app/scripts/build-sku-index.d.ts` so TypeScript can type-check test imports. Created `app/src/lib/buildSkuIndex.test.ts` (12 tests, `@vitest-environment node`) validating slug generation, index shape, history ordering, removed-SKU null price, multi-scope handling, and empty-input edge cases. Updated `package.json` build script to `node scripts/build-sku-index.js && vite build`. Added `app/public/sku-index.json` to `.gitignore` (build artifact). Output: 4,413 SKU families, 2,603 KB. 185 tests green; lint, typecheck, build all pass.
- Decisions: Pure functions are exported from the script file (not a separate module) and covered by a `@vitest-environment node` test — avoids introducing a parallel TypeScript module. `sku-index.json` is gitignored (regenerated each build from live data); the deploy workflow already runs `npm run build` after copying data. Skipped openai-eu.json (empty object, zero keys) silently — logged via the existing `Object.keys(raw).length === 0` guard.
- Noticed for later: sku-index.json is 2.6 MB uncompressed; gzip should reduce it to ~500 KB. If size becomes a concern, consider splitting by scope or dropping history from the index (lazy-load it separately). Issues #69 and #72 remain the next picks.
