...
## 2026-07-21T10:55Z — run 2026-07-21-2
- Task: CI fix — PR #88 (issue #83 E2.3 region comparison overlay)
- Did: PR #88 had failing lint due to two incomplete implementations merged together in `SkuPage.tsx` and `SkuPage.test.tsx`. Fixed `SkuPage.tsx`: (1) removed the old incomplete `MultiRegionTooltip` function (missing closing `</div>`, `)`, `}`) that was left before the new complete version, causing a parse error at line 160; (2) removed duplicate `const primaryRegion` declaration and the associated early return (which also violated react-hooks/rules-of-hooks); (3) fixed `chartData` (undefined) → `primaryPoints` in `canCompare`; (4) removed `const color = directionColor(getChartDirection(regionPoints))` which referenced an undefined `regionPoints`; (5) merged the two duplicate `<h2>` heading elements into one that handles both compare-mode and region-selector modes; (6) replaced the old `{chartData.length >= 2 && (...)}` chart section (missing its closing `)}`) and the new `{primaryPoints.length >= 2 && (...)}` chart section with a single correct block that integrates both compare-mode (multiChartData + eligibleRegionNames) and region-selector (mergedData + selectedRegions). Fixed `SkuPage.test.tsx`: (1) merged duplicate `@testing-library/react` import (adding `within`); (2) added missing `})` closing the `it('can deactivate compare mode…')` test and `describe('region comparison')` block before `describe('region selector')`. 240 tests green; lint, typecheck, build all pass.
- Decisions: Removed the early-return guard `if (!entry.regions.length) return null` entirely (instead of moving it) — with all hooks before it removed, the component safely renders the empty state when `entry.regions` is empty. All compare-mode tooltip calls now use the new `MultiRegionTooltip` signature (`perRegionData`, `entry`) rather than the old `unitOfMeasure`-only signature (Tooltip is mocked in tests so this has no test impact).
- Noticed for later: `PriceHistoryChart.tsx` remains orphaned (noted in previous runs); issue #84 (trend summary card) is the next claude-ready task.
## 2026-07-21T00:00Z — PM run
- Triage: #89 "Issue issue-83-sku-page-region-comparison" (P1) had malformed body (Copilot Chat dump referencing non-existent SkuPage.tsx syntax errors). Rewrote title and body to clarify ambiguity; relabeled `claude-ready` → `needs-human` pending human clarification.
- Created: #83 E2.3 and #84 E2.4 already exist from prior PM run (2026-07-20T16:30Z); queue holds 2 open claude-ready issues after #89 triage.
- Rationale: E1 is complete; E2 is now Current Epic. All E2.1 subtasks shipped or in-flight (#75, #77, #76 closed; #82 just shipped). E2.2 (region selector) and E2.3/E2.4 (comparison overlay + trend summary) are queued and ready. Queue is sustainable at 2/5; no new issues created. Next PM run will create issues only if queue drops below 5 and E2 progress continues.

## 2026-07-21T14:30Z — run 2026-07-21-3
- Task: #84 E2.4 — Trend summary card on SKU detail page (30/90-day direction, first-seen, change count)
- Did: Created `lib/trendSummary.ts` with `computeTrendSummary` pure function that calculates 30-day and 90-day trend windows (direction, % change), first-seen date, change count, and current price. Each window has a status: `ok` (≥2 points in window), `insufficient` (<2 total), or `stale` (data exists but too old for window). Created `TrendSummaryCard` component that renders a two-column stat grid (stacks to 1 column on mobile ≤640px) with direction badges colored per ADR 0002 (`#34D399` drop, `#F87171` increase, `#FBBF24` stable). Wired `TrendSummaryCard` into `SkuPage` using the cheapest region as primary region. Added 15 unit tests in `trendSummary.test.ts` (insufficient, stale, directions, metadata, primary-region isolation) and 14 component tests in `TrendSummaryCard.test.tsx` (render, all badge states, current price, labels). Fixed issue #89's stale `claude-ready` label (PM intended `needs-human`). 266 tests green; lint, typecheck, build all pass.
- Decisions: Accepted a `now: Date = new Date()` injectable parameter on `computeTrendSummary` for deterministic testing. "Stale" threshold is simply <2 points in the window — consistent with "Not yet determined" language in the issue spec. TrendSummaryCard uses `h2` (same level as SkuHistory heading) to match the visual hierarchy; updated 3 existing SkuPage tests that used `getByRole('heading', level 2)` to use `getAllByRole` now that there are two h2s on the page.
- Noticed for later: `PriceHistoryChart.tsx` remains orphaned.

## 2026-07-22T00:00Z — PM run
- Triage: none — no unlabeled or needs-human open issues
- Created: #91 E2.5 Auto-generated OpenGraph preview image per SKU page (P2)
- Rationale: Queue had 0 open claude-ready issues; E2 epic is complete (E2.1a–E2.4 shipped). Created final E2 subtask (#91) to finish the epic. Per vision constraints, not starting E3 without human advancing the Current Epic line. Queue now at 1/5 with one issue ready; next PM run holds until queue fills or human updates product-vision.md.

## 2026-07-22T10:44Z — run 2026-07-22-1
- Task: #91 E2.5 — Auto-generated OpenGraph preview image per SKU page
- Did: Created `app/scripts/build-og-images.js` (ESM, no new deps) with exported `slugify`, `computeOgData`, and `generateOgSvg` pure functions. Generates one 1200×630 SVG per SKU using ADR 0002 tokens (dark `#0B1120` bg, `#38BDF8` accent, `#34D399`/`#F87171` direction colors). SVG includes SKU name (adaptive font-size 22–52px), current price, 30-day trend direction + magnitude, and cheapest region. Wired into `npm run build` after `build-sku-index.js`; output goes to `app/public/og-images/` (gitignored, copied to `dist/` by Vite). Created `app/src/lib/ogImage.ts` (TypeScript mirror) with `slugify`, `computeOgData`, and `ogImageUrl` exported for `SkuPage.tsx` and tests. Updated `SkuPage.tsx` meta useEffect to set `og:image` (and restore on unmount) when a known SKU entry is loaded. Added `app/public/og-images/` to `.gitignore`. Added 9 tests across 3 describe blocks (slugify, computeOgData for 5 cases, ogImageUrl). 275 tests green; lint, typecheck, build all pass; 4413 SVG files generated.
- Decisions: Used SVG format (no new dependencies) rather than PNG — the issue explicitly lists SVG first. Adaptive font-size keeps all SKU names (up to 64 chars) within the 1080px usable width. Build script re-implements trend computation in plain JS (can't import TS); TypeScript version delegates to existing `computeTrendSummary`. Noted in PR: hash-based routing means most social crawlers see `index.html` and won't navigate to a hash-routed SKU URL — the og:image is visible to JS-executing unfurlers (Slack, Teams) but not to pure-HTML crawlers.
- Noticed for later: If PNG is needed for broader platform support, a future issue can add `@resvg/resvg-js` or `sharp` to convert the SVGs at build time.

## 2026-07-23T00:00Z — PM run
- Triage: none — no unlabeled or needs-human open issues
- Created: #93 E3.1 Daily digest generation (changelog-writing skill), #94 E3.2 Digest archive page, #95 E3.3 RSS/Atom feed of digests
- Rationale: E2 complete (all sub-tasks shipped as of 2026-07-22); Current Epic advanced to E3 per product-vision.md. Created the first three E3 sub-tasks in order: digest generation (core artifact), archive page (web UI discovery), and feed (subscription). Queue now at 3/5 open claude-ready issues; next PM run will hold unless developer runs close issues quickly.

## 2026-07-23T10:45Z — run 2026-07-23-1
- Task: #93 E3.1 Daily digest generation (changelog-writing skill)
- Did: Created `scripts/generate-digest.js` (ESM, no new deps) with exported pure functions: `computePctChange`, `formatDigestPrice`, `computeDigestData`, `renderMarkdown`, `renderJson`. Reads the most recent date in `data/diffs/`, produces `data/digests/<YYYY-MM-DD>.md` and `.json`. Markdown digest includes headline, plain-language summary (drops/increases/new SKUs/removed), and top 3 movers per scope section sorted by |%| descending with drops before increases at equal magnitude; changes under 0.5% filtered as rounding noise. JSON format mirrors the same data for downstream consumers (E3.2/E3.3). Created `app/src/lib/digest.ts` (TypeScript mirror) and `app/src/lib/digest.test.ts` with 30 tests covering all branches (computePctChange, formatDigestPrice, computeDigestData with 13 cases, renderMarkdown with 9 cases, renderJson with 2 cases). Integrated into `scripts/fetch_prices.py` via subprocess call when changes are detected. Updated README with "Daily digests" section. Bootstrapped first digest: `data/digests/2026-07-15.md` (initial load: 9,904 new SKUs, no price changes). 305 tests green; lint, typecheck, build all pass.
- Decisions: Followed the same build-sku-index.js pattern (JS script + TS mirror tested by Vitest) rather than a separate Node test runner — keeps all tests in one `npm run test` command. The `data/digests/` files are committed artifacts (not gitignored) since they are the canonical digest archive per the issue spec. Digest generation runs only when `any_changes` is True in `fetch_prices.py` — avoids regenerating a prior day's digest on every no-change poll run.
- Noticed for later: `PriceHistoryChart.tsx` remains orphaned (noted since run 2026-07-20-9); still a valid cleanup candidate for a future issue.

## 2026-07-23T14:44Z — run 2026-07-23-2
- Task: #94 E3.2 Digest archive page (browsable by date, biggest movers per day)
- Did: Created `app/src/pages/DigestArchive.tsx` with loading/error/empty states, date-sorted entry list (newest first), direction-coded count badges (drops/increases/new/removed), and top-3 cross-scope movers per entry. Created `app/src/pages/DigestArchive.css` using only ADR 0002 tokens. Added `{ view: 'digests' }` to `useRoute.ts`/`parseRoute` and lazy-loaded `DigestArchive` in `Router.tsx` (own 4 kB chunk). Added `loadDigests.ts` to fetch `data/digests/manifest.json` then individual digest JSON files, sorting results newest-first. Added the `buildDigestsManifest()` function and `/digests/manifest.json` endpoint to the Vite dev plugin in `vite.config.ts`. Added a "Digests" nav link in the App header (`#/digests`). Added 4 tests in `loadDigests.test.ts` and 13 in `DigestArchive.test.tsx`; added 3 `useRoute` tests. Updated README "Daily digests" section. 325 tests green; lint, typecheck, build all pass.
- Decisions: Created `app/src/pages/` directory (new, per issue spec) rather than placing the component in `components/` — pages are top-level routed views, not reusable UI components. Lazy-loaded `DigestArchive` in `Router.tsx` following the same pattern as `SkuPage` — keeps the main bundle lean. Placed the "Digests" nav link before `LastUpdatedBadge` in the header so badge stays rightmost; no `margin-left: auto` needed since the badge already owns that alignment. Top-3 movers aggregated across all sections (not per-section) for the archive view — more meaningful at-a-glance than per-scope lists.
- Noticed for later: `PriceHistoryChart.tsx` remains orphaned (noted since run 2026-07-20-9). The digest archive will become more useful once more than one day's digest exists (currently only 2026-07-15).

## 2026-07-23T17:53Z — run 2026-07-23-3
- Task: #95 E3.3 RSS/Atom feed of digests (for feed readers)
- Did: Created `scripts/generate-feed.js` (ESM, no new deps) with exported pure functions `escapeXml`, `formatRssDate`, `computeEntryText`, `renderAtom`, `renderRss`. Reads all `data/digests/*.json` files (newest-first), produces `data/feed.atom` (Atom 1.0) and `data/feed.rss` (RSS 2.0). Each entry includes date, title ("Azure pricing changes for YYYY-MM-DD"), plain-text summary with top movers, and link to `#/digests/YYYY-MM-DD`. Integrated into `fetch_prices.py` alongside `generate-digest.js` (runs when `any_changes` is True). Created `app/scripts/build-feed.js` that copies `data/feed.atom` and `data/feed.rss` to `app/public/` at build time; added to `npm run build` pipeline. Created `app/src/lib/feed.ts` (TypeScript mirror) and `app/src/lib/feed.test.ts` with 32 tests covering all pure functions. Updated `.gitignore` to exclude `app/public/feed.atom` and `app/public/feed.rss` (generated artifacts). Bootstrapped `data/feed.atom` and `data/feed.rss` from the existing 2026-07-15 digest. Updated README with "Subscribe" section listing Atom and RSS URLs. 357 tests green (32 new); lint, typecheck, build all pass.
- Decisions: Committed `data/feed.atom` and `data/feed.rss` as repo artifacts (same as digest files) so they exist in the repo before any build runs. `app/public/feed.{atom,rss}` are gitignored (build-time copies). Used `generatedAt` from the digest JSON as the Atom `<updated>` timestamp when available — more precise than midnight UTC. Atom 1.0 was chosen as the primary format (`feed.atom` self-link); RSS 2.0 added for broader reader compatibility.
- Noticed for later: `PriceHistoryChart.tsx` remains orphaned (noted since run 2026-07-20-9).

## 2026-07-24T00:00Z — PM run
- Triage: none — no unlabeled or needs-human open issues
- Created: #100 E3.4 "This week in Azure pricing" weekly rollup page, #101 needs-human E3.5 Belgium Central region proposal, #102 cleanup: remove orphaned PriceHistoryChart
- Rationale: Queue was empty (0 open claude-ready); all E3.1–E3.3 issues have shipped. Created E3.4 (weekly rollup, P2) as the next epic sub-task per the Current Epic E3 sequence. #101 is a needs-human verification task for E3.5 region addition per the coverage roadmap constraints. #102 addresses a journal-flagged cleanup (PriceHistoryChart orphaned since 2026-07-20-9). Queue now at 3/5; next PM run holds until developer runs close issues.

## 2026-07-24T10:37Z — run 2026-07-24-1
- Task: #100 E3.4 "This week in Azure pricing" weekly rollup page + markdown
- Did: Created `app/src/lib/weeklyDigest.ts` (pure functions: `getIsoWeekLabel`, `getIsoWeekBounds`, `filterDigestsForWeek`, `computeWeeklyDigest`, `renderWeeklyMarkdown`, `renderWeeklyJson`) and 24 unit tests. Created `WeeklyDigest.tsx` page that loads daily digests via `loadDigests()`, filters to the current ISO week, aggregates into a summary card (drops/increases/new/removed, services/regions affected), top-5 movers, and per-service breakdown — with loading skeleton, empty state ("No pricing data this week"), and error state. Added lazy-loaded `#/weekly` route to `useRoute.ts` and `Router.tsx`; added "This week" nav link to `App.tsx`. Created `app/scripts/build-weekly-digest.js` (bootstraps `data/digests/YYYY-Www.json` + `.md` for each ISO week with daily data). Created `app/scripts/build-digests-data.js` (copies digest JSON files to `app/public/data/digests/` at build time + generates manifest) and wired it into `npm run build` — this also fixes the pre-existing production gap where DigestArchive had no digest data in `app/dist/`. Bootstrapped `data/digests/2026-W29.json` and `.md`. 395 tests green; lint, typecheck, build all pass.
- Decisions: Page filters by ISO week (not rolling 7 days) because the `YYYY-Www` artifact naming implies week-bounded aggregation and makes the "this week" concept unambiguous regardless of when in the week you visit. `build-weekly-digest.js` skips already-existing weekly files to be idempotent (human wires it into `scripts/fetch_prices.py` later, per issue out-of-scope). `build-digests-data.js` placed in `app/scripts/` (not `scripts/`) — it's a build-time asset copy, not pipeline logic, and follows the same pattern as `build-feed.js`. Weekly page is its own lazy chunk (7.71 kB).
- Noticed for later: `PriceHistoryChart.tsx` remains orphaned (issue #102 targets this).

## 2026-07-24T14:15Z — run 2026-07-24-2
- Task: #102 cleanup: remove orphaned PriceHistoryChart component
...
## 2026-07-25T10:05Z — run 2026-07-25-1
- Task: #108 E3.6 needs-human: Add North Europe and France Central regions to price tracking
- Did: Issue #108 is a verification/planning issue whose DoD asked a human to validate prerequisites and then have me create a claude-ready implementation issue. The project owner validated ("Agree, I validate"). Created #109 "E3.6 implement: Add North Europe and France Central regions to price tracking" (claude-ready, P2) with explicit frozen-zone exception for `scripts/fetch_prices.py`, exact SCOPES entries for four new scopes (`vm-northeurope`, `storage-northeurope`, `vm-francecentral`, `storage-francecentral`), and README update requirement. Closed #108 as completed.
- Decisions: Followed the identical pattern as E3.5 (#101 → #105). France Central is already partially in scope via `openai-eu`; the new scopes add VM and Storage coverage for that region. North Europe OpenAI coverage left out-of-scope pending separate verification (consistent with issue DoD).
- Noticed for later: nothing new.

## 2026-07-26T00:00Z — PM run
- Triage: none — no unlabeled or needs-human open issues
...
## 2026-07-27T11:45Z — run 2026-07-27-1
- Task: #116 E3.1 Daily digest generation (quality-bar fix: scope display names)
- Did: Discovered #116, #117, #118, #119 are duplicates of #93, #94, #95 (all completed 2026-07-23); closed #117, #118, #119 as duplicates. Found a real quality gap under #116's DoD ("region specificity"): `SCOPE_DISPLAY_NAMES` in `scripts/generate-digest.js` and `app/src/lib/digest.ts` was missing entries for the 8 scopes added in E3.5–E3.7 (Belgium Central, North Europe, France Central, Sweden Central), causing raw scope keys like "storage-swedencentral" to appear in digest markdown/headers. Also added `belgiumcentral` to `REGION_DISPLAY_NAMES`. Added 2 new tests covering all newer scope display names and all tracked region display names. 386 tests green; lint, typecheck, build all pass.
- Decisions: Treated #116 as a quality-bar fix (not a duplicate close) because the underlying E3.1 implementation failed one of its own DoD requirements ("region specificity") once later region-addition issues shipped. Closing #117, #118, #119 as duplicates is housekeeping within the same task, not a separate task.
- Noticed for later: The PM run created duplicate issues for E3.1–E3.3; the root cause appears to be that the PM's backlog-triage context did not include these closed issues. Worth checking whether the PM playbook should scan closed issues before recreating sub-tasks.

## 2026-07-28T00:00Z — PM run
- Triage: none — no unlabeled or needs-human open issues
...
## 2026-08-01T17:22Z — run 2026-08-01-3
- Task: #123 E4.3: Show PAYG vs 1yr vs 3yr vs savings-plan pricing side by side on SKU detail page
...
- Noticed for later: nothing new.
## 2026-08-03T08:00Z — run 2026-08-03-1
- Task: #122 E4.2 implement: Add savings-plan pricing as a tracked dimension
- Did: Modified `scripts/fetch_prices.py` `index_by_key` to extract the 1-year savings-plan price from the nested `savingsPlan` array of each Consumption API response item and store it as `savingsPlanPrice` alongside the other tracked fields in the snapshot entry. Storage and OpenAI items return an empty `savingsPlan` array so they are unaffected. Updated `app/scripts/build-sku-index.js` and its TypeScript mirror `app/src/lib/skuIndex.ts` to propagate `savingsPlanPrice` from diff items (both `added` and `changed` entries) into `SkuHistoryPoint.savingsPlanPrice` — the field was already declared with a comment noting it would be populated by E4.2. Added 3 new tests in `skuIndex.test.ts` covering: savings-plan price carried through `added` diffs, through `changed` diffs, and omitted when absent. Updated README to document the new savings-plan pricing dimension. 398 tests green; lint, typecheck, build all pass.
- Decisions: Stored only the 1-year savings-plan price in `savingsPlanPrice` (not 3yr) — the existing `SkuHistoryPoint` interface has a single `savingsPlanPrice` field and the discount chart in SkuPage.tsx labels it "Savings plan" (singular). On the next pipeline run after this PR merges, VM snapshot entries will differ from the stored ones (they gain `savingsPlanPrice`), triggering a one-time "changed" diff for every VM SKU that has savings-plan rates — an expected and documented side effect of extending tracked fields (Option B from ADR 0004).
- Noticed for later: PRs #134 (E4.1 design ADR), #126 (E4.3 pricing tiers UI), #135 (previous journal) are all open with passing CI awaiting human review.
## 2026-08-02T21:15Z — run 2026-08-02-4
- Task: #121 E4.1 needs-human: Design proposal for Reservation pricing support
- Did: Created `docs/adr/0004-reservation-pricing.md` (status: proposed). Documents three design options — Option A (new parallel scope files, minimal pipeline change), Option B (augment existing snapshot entries), Option C (separate data/reservation/ directory) — with pros/cons for each. Describes exact API query patterns for Reservation and Savings Plan price types, sample response shapes, and the Phase 1 scope recommendation (VMs only across all 5 current VM regions, 1yr and 3yr terms). Notes that `reservationTerm` is already in `KEY_FIELDS` and `SkuRegion` optional fields are already declared in `skuIndex.ts`. Documents ADR/data-contract impact per option. All 395 tests green; lint, typecheck, build all pass.
- Decisions: Framed as a proposal (status: proposed) rather than an accepted ADR — the human must choose an option before E4.2 implementation begins. Presented all three options without prescribing a final choice, per the issue's out-of-scope constraint ("just present the options clearly"). Noted that Option A requires the least pipeline change and leaves the existing data contract untouched, which may guide the human's decision.
- Noticed for later: Issue #133 (P1, "Solve this PR, merge does not work") refers to PR #126, which currently shows `mergeStateStatus: CLEAN` and CI passing — the merge blocker may have already been resolved by prior rebases; a human should verify and merge #126 directly.
